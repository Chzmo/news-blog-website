...
function Card({ image, content, author, title, hover_color }) {
	const [hover, setHover] = useState("text-[#3d3d3d]");

	return (
		<Link className='flex flex-col gap-2 w-full '>
			...
			{title ? (
				<>
					<Link
						className={`'flex w-full text-[#3d3d3d] font-semibold text-xl' ${hover_color}`}>
						<h5>{title}</h5>
					</Link>
				</>
			) : (
